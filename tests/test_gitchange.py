...
class MyTestCase(unittest.TestCase):
    def testGetFilename(self):
        expected_result = "firewall/project1.yaml"
        gitchange = GitChange("firewall", "project1")
        result = gitchange.get_filename()
        self.assertEqual(result, expected_result)

    def testGetCommitMsg(self):
        #No changers
        gitchange = GitChange("testresource", "testproject")
        expected_result = "Testresource updated in testproject by UNKNOWN"
        ...
        expected_result = "Testresource updated in testproject by ['testuser', 'anothertestuser']"
        self.assertEqual(gitchange.get_commit_message(), expected_result)
